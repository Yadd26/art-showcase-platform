    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md p-8">
            <div class="flex justify-between items-start mb-6">
                ...
                <form action="{{ route('curator.challenges.destroy', $challenge) }}" 
                      method="POST" 
                      onsubmit="return confirm('Delete this challenge? This action cannot be undone.')">
                    @csrf
                    @method('DELETE')
            ...
            <form action="{{ route('curator.challenges.update', $challenge) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" 
                              ...
                              rows="6"
                              required
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-900 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $challenge->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="rules" 
                              ...
                              rows="6"
                              required
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-900 focus:border-transparent @error('rules') border-red-500 @enderror">{{ old('rules', $challenge->rules) }}</textarea>
                    @error('rules')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" 
                               id="prize_pool" 
                               name="prize_pool" 
                               value="{{ old('prize_pool', $challenge->prize_pool) }}"
                               required
                               min="0"
                               step="1000"
                    ...
                    <div>
                        ...
                        <input type="number" 
                               id="max_winners" 
                               name="max_winners" 
                               value="{{ old('max_winners', $challenge->max_winners) }}"
                               required
                               min="1"
                               max="10"
                ...
                @if($challenge->banner_image)
                    <div class="mb-6">
                        ...
                        <img src="{{ Storage::url($challenge->banner_image) }}" 
                             alt="Current banner"
                             class="w-full max-w-2xl h-48 object-cover rounded-lg">
                    </div>
                @endif
                ...
                <div class="mb-6">
                    <label for="banner_image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $challenge->banner_image ? 'Change Banner Image' : 'Banner Image' }}
                    </label>
                    <input type="file" 
                           id="banner_image" 
                           name="banner_image" 
                    ...
                    <p class="mt-1 text-xs text-gray-500">Leave empty to keep current banner. JPG, PNG, or WEBP. Max 5MB.</p>
                    @error('banner_image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-700">Challenge is active</span>
                    </label>
                </div>
